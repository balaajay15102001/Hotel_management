...
<%
  ...
  try{
    ...
    ResultSet owner_page_details = stmt.executeQuery("select * from hotel_user_login;"); 
            int f=0;
            while(owner_page_details.next()){
                ...
                out.print(email+" "+pass);
                 if(owner_page_details.getString(2).equals(email) && owner_page_details.getString(3).equals(pass)){
                  ...
                  String url=session.getAttribute("urlname").toString();
                  if(url.equals("null")){
                    response.sendRedirect("http://localhost:8080/hotel_management/home_page.jsp");
                    f=1;
                  }
                  else{
